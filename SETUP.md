...
We have included a cross-platform script to handle everything for you on both Windows and macOS.

### Option A: The Easy Way (Recommended - Cross-Platform)
Open your terminal (Terminal on macOS, or Command Prompt/PowerShell on Windows) in the project folder and run:
```bash
node start.js
```
...
### Option B: Windows-Only Shortcut
If you are on Windows, you can also simply **double-click** the `start_app.bat` file.

### Option C: The Manual Way (If script fails)
...
*   **Port Check**: Ensure ports 5001 and 5173 are free.
