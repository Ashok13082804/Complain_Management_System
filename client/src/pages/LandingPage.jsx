...
const LandingPage = () => {
    ...
    const fetchHistory = async () => {
        if (user && !String(user.id).startsWith('demo-')) {
            try {
                const res = await axios.get(`http://localhost:5001/api/complaints/my-history?userId=${user.id}`);
                setHistory(res.data);
            } catch (err) {
            ...
            try {
                ...
                console.error("Failed to parse complaintHistory from localStorage:", e);
    ...
    const checkStatus = async (e) => {
        ...
        if (!statusId) return;
        try {
            const res = await axios.get(`http://localhost:5001/api/complaints/public/status/${statusId}`);
    ...
    const clearHistory = async () => {
        if (window.confirm("Are you sure you want to clear your complaint history?")) {
            if (user && !String(user.id).startsWith('demo-')) {
                try {
                    await axios.delete('http://localhost:5001/api/complaints/my-history/clear', { data: { userId: user.id } });
                } catch (err) {
